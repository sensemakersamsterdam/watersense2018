// TBD https://api.ipify.org/

/***************************
   All includes come here.
 ***************************/
#include <ESP8266WiFi.h>
#include <WiFiClient.h>
#include <ESP8266WebServerSecure.h>
#include <ESP8266mDNS.h>
#include <time.h>
//#include <WiFiUdp.h>
#include <ArduinoOTA.h>
//#include <NTPClient.h>
#include "FS.h"

/*********************
   Forward prototypes
 *********************/
char * getCLOB(String);


/*********************************************
   Global constants, vars, objects come here.
 *********************************************/
long st;

const char* ssid = "matanga2.4";
const char* password = "12hoppetee123";

const char* www_username = "sensemaker";
const char* www_password = "secret";

BearSSL::ESP8266WebServerSecure server(443);

//WiFiUDP ntpUDP;
//NTPClient timeClient(ntpUDP, "europe.pool.ntp.org", 3600, 24 * 69 * 60 * 1000L);

static const char serverCert[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)EOF";

static const char serverKey[] PROGMEM =  R"EOF(
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
)EOF";


void setup() {
  pinMode(LED_BUILTIN, OUTPUT);
  digitalWrite(LED_BUILTIN, HIGH );

  Serial.begin(115200);
  Serial.println(F("\nBooting Rain Gauge"));

 wifiSetup();

  configTime(3600, 0, "pool.ntp.org", "time.nist.gov");
  Serial.println(time(nullptr));
  while(time(nullptr) < 100000) {
    Serial.print("t"); delay(500);
  }

 SPIFFS.begin();

  // arduinOTASetup();


  ArduinoOTA.setHostname("SenseRain01");
  webServerSetup();

//  timeClient.begin();
//  timeClient.update();

//  Serial.println("HTTPS server started on " + timeClient.getFormattedTime() + "." );
  time_t now = time(nullptr);


  Serial.println("HTTPS server started on " + String(ctime(&now)) + "." );

  digitalWrite(LED_BUILTIN, LOW );

  st = millis();
}



void loop() {
  //  ArduinoOTA.handle();
  yield();
  server.handleClient();
  yield();
  if (millis() - st > 10000) {
  time_t now = time(nullptr);


  Serial.println("HTTPS running on " + String(ctime(&now)) + "." );    st = millis();
  }
}
